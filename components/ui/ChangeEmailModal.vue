          ...
          <div class="relative z-10">
            <div class="flex items-center gap-3 mb-6">
              <div class="w-10 h-10 bg-indigo-500/10 rounded-full flex items-center justify-center">
                <mail-icon class="w-5 h-5 text-indigo-500" />
              </div>
              <h3 class="text-xl font-display font-bold text-white">Change Email</h3>
            </div>

            <form @submit.prevent="handleSubmit" class="space-y-4">
              <div class="space-y-1">
                <label class="text-sm font-medium text-slate-300">New Email Address</label>
                <input 
                  v-model="newEmail"
                  type="email" 
              ...
              <div class="flex gap-3 pt-2">
                ...
                >
                  <span v-if="!isLoading">Update Email</span>
...
const handleSubmit = async () => {
  ...
  try {
    const result = await $fetch('/api/admin/change-email', {
      method: 'POST',
      body: {
    ...
    error.value = e.data?.message || e.statusMessage || 'Failed to change email'
  } finally {
